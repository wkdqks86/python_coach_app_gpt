...
export default function AppStudio(){
  ...
  const [loadingAction,setLoadingAction]=useState<'run'|'grade'|null>(null)
  ...
  const loadingPopup=initialConnecting||loadingAction?<div className="loading-overlay" role="status" aria-live="polite"><section><span className="loading-spinner" aria-hidden="true"/><p>{initialConnecting?'SERVER CONNECTION':loadingAction==='run'?'CODE EXECUTION':'ANSWER CHECK'}</p><h2>{initialConnecting?'서버 연결 중':loadingAction==='run'?'코드를 실행하고 있어요':'답안을 채점하고 있어요'}</h2><small>{initialConnecting?'학습 기록을 불러오고 있어요. 무료 서버가 잠시 멈춰 있었다면 최대 1분 정도 걸릴 수 있어요.':'잠시만 기다려 주세요. 작업이 끝나면 결과를 바로 보여 드릴게요.'}</small></section></div>:null
